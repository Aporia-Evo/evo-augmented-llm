            ...
            for _ in range(self.activation_steps):
                ...
                for node in genome.nodes:
                    ...
                    write_selectivity_logit = (
                        ...
                        - (0.85 * query_signal)
                        - (0.6 * max(0.0, key_query_cos))
                        - (0.45 * projection_write_signal)
                        - (0.35 * query_collapse_signal)
                    )
                    write_eligibility = 0.5 + (0.5 * math.tanh(write_selectivity_logit))
                    write_selectivity = 1.0 + (0.35 * math.tanh(write_selectivity_logit))
                    beta_gate = 0.55 + (0.9 * write_eligibility)
                    beta_t = beta_mix * (1.0 - (0.3 * query_signal)) * write_selectivity * beta_gate
